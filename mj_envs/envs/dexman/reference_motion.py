...
class ReferenceMotion():
    ...
    def check_format(self, reference):
        ...
        """
        assert 'time' in self.reference.keys(), "Missing keys in reference"
        assert 'robot' in self.reference.keys(), "Missing keys in reference"
        assert 'object' in self.reference.keys(), "Missing keys in reference"
        assert self.reference['robot'].ndim == 2, "Check robot reference, must be shape(N, n_robot_jnt)"
        assert self.reference['object'].ndim == 2, "Check object reference, must be shape(N, n_object_jnt)"
        if 'robot_init' in self.reference.keys():
            assert self.reference['robot_init'].ndim == 1, "Check robot_init reference, must be shape(n_robot_jnt)"
            assert self.reference['robot_init'].shape[0] == self.reference['robot'].shape[1], "n_robot_jnt different between motion and init "
        if 'object_init' in self.reference.keys():
            assert self.reference['object_init'].ndim == 1, "Check object_init reference, must be shape(n_object_jnt)"
            assert self.reference['object_init'].shape[0] == self.reference['object'].shape[1], "n_object_jnt different between motion and init "
    ...
    def find_timeslot_in_reference(self, time):
        ...
        time = np.around(time, 4) # round to help with comparisions
    ...
    def get_reference(self, time):
        ...
        if self.type == ReferenceType.FIXED:
            ...
            print("======>", ind, ind_next, self.horizon)
            if ind == ind_next:
                ...
                if self.robot_horizon>1:
                    robot_ref = blend*self.reference['robot'][ind]+(1-blend)*self.reference['robot'][ind_next]
                else:
                    robot_ref = self.reference['robot'][0]
                if self.object_horizon>1:
                    object_ref = blend*self.reference['object'][ind]+(1-blend)*self.reference['object'][ind_next]
                else:
                    object_ref = self.reference['object'][0]
